...
problem = mlrose.DiscreteOpt(length = 16, fitness_fn = fitness, maximize = True, max_val = 2)
...
sa_best_state, sa_best_fitness, sa_fitness_curve = \
               mlrose.simulated_annealing(problem, max_attempts = 100, max_iters = 1000,\
                                          random_state = 1, curve = True)
t1 = time.time()
times[1] = t1-t0
...
mimic_best_state, mimic_best_fitness, mimic_fitness_curve = \
                  mlrose.mimic(problem, max_attempts = 100, max_iters = 1000,\
                               random_state = 1, curve = True)
t1 = time.time()
times[3] = t1-t0
